...
<div class="clearfix">
	<div <?php orbis_field_class( array( 'control-group' ), 'orbis_registration_company_id' ); ?> style="float: left; margin-right: 20px;">
		...
		<input placeholder="Select company" type="text" name="orbis_registration_company_id" value="<?php echo esc_attr( $entry->company_id ); ?>" class="orbis-id-control orbis-company-id-control"  style="width: 500px;" data-text="<?php echo esc_attr( $entry->company_id ); ?>" tabindex="0" />
	</div>
	...
	<div <?php orbis_field_class( array( 'control-group' ), 'orbis_registration_project_id' ); ?> style="float: left;">
		...
		<input placeholder="Select project" type="text" name="orbis_registration_project_id" value="<?php echo esc_attr( $entry->project_id ); ?>" class="orbis-id-control orbis-project-id-control"  style="width: 500px;" data-text="<?php echo esc_attr( $entry->project_id ); ?>" />
	</div>
...
<div class="clearfix">
	...
	<div <?php orbis_field_class( array( 'control-group', 'col' ), 'orbis_registration_time' ); ?> style="width: 40%; float: left;">
    	...
    	<div class="input-append help-inline">
			<input size="2" style="width: auto;" type="text" name="orbis_registration_hours" value="<?php echo empty( $entry->time ) ? '' : esc_attr( orbis_time( $entry->time, 'H' ) ); ?>" />
		    <span class="add-on">uren</span>
    	</div>
    	
    	<div class="input-append help-inline">
			<input size="2" style="width: auto;" type="text" name="orbis_registration_minutes" value="<?php echo empty( $entry->time ) ? '' : esc_attr( orbis_time( $entry->time, 'M' ) ); ?>" />
